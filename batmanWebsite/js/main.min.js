...
characterSelect.forEach((select, index) => {
  select.addEventListener('click', () => {
    ...
    roleCharacter.classList.add('fade-out');
    descriptionCharacter.classList.add('fade-out');

    // Cambiar textos inmediatamente (no dependen de carga)
    ...
    // Esperar a que la NUEVA imagen cargue antes del fade-in
    const newImg = new Image();
    newImg.src = character.img;

    const fadeIn = () => {
      characterImg.src = newImg.src;

      // Pequeño delay para que el DOM actualice el src antes del fade-in
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          roleCharacter.classList.remove('fade-out');
    ...
    if (newImg.complete) {
      // Ya estaba en caché
      setTimeout(fadeIn, FADE_DURATION);
    } else {
      // Esperar carga real
      newImg.addEventListener('load', fadeIn);
    }
  });
});

// ═══════════════════════════════════════════════════════════════
//  LAZY LOAD DE characters.min.js con IntersectionObserver
// ═══════════════════════════════════════════════════════════════
if (charactersSection) {
  const lazyObserver = new IntersectionObserver((entries) => {
    ...
    script.src    = '../js/characters.min.js';
    script.onload = () => initCharacterSelector();
    document.head.appendChild(script);

  }, { rootMargin: '200px', threshold: 0 });

  lazyObserver.observe(charactersSection);
}
...
// ═══════════════════════════════════════════════════════════════
//  BATMOBILE con IntersectionObserver sobre sentinel
// ═══════════════════════════════════════════════════════════════
...
const mobileObserver = new IntersectionObserver(([entry]) => {
  ...
  if (shouldShow === batMobileVisible) return;
